...
export default function EpcBulkPanel({ t }) {
  ...
  return (
    <div>
      <div className="mb-3">
        <div className="text-sm font-semibold text-zinc-900">{t('bulkImport')}</div>
        <div className="mt-1 text-xs text-zinc-600">{t('bulkImportHint')}</div>
      </div>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_auto] md:items-end">
          <div>
            ...
            <div className="mt-2 flex items-center gap-2 text-xs text-zinc-700">
              <input id="dryRunEpc" type="checkbox" checked={dryRun} onChange={(e) => setDryRun(e.target.checked)} />
              <label htmlFor="dryRunEpc">{t('dryRun')}</label>
            </div>
          </div>
      ...
      {result ? (
        <div className="mt-4 rounded-xl border border-zinc-200 bg-white p-4">
          ...
          <pre className="mt-2 overflow-auto rounded-lg bg-zinc-50 p-3 text-[11px] text-zinc-800">{JSON.stringify(result, null, 2)}</pre>
        </div>
      ) : null}
    </div>
  );
